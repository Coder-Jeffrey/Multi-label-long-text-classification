...
def load_data():
    entity2id = {}
    relation2id = {}
    with open('books_entity2id.txt', 'r', encoding='utf-8') as f1, open('books_relation2id.txt', 'r', encoding='utf-8') as f2:
        lines1 = f1.readlines()
        lines2 = f2.readlines()
        for line in lines1:
    ...
    with open('books_all_triple.pickle', 'rb') as f:
    ...
    entity_embed = {}
    with open('books_entity_embeddings.txt', 'r', encoding='utf-8') as f1:
        lines1 = f1.readlines()
        for line in lines1:
            line = line.strip().split('\t')
            if len(line) != 2:
...
def get_knowledge(vocab, PPMI):
    ...
    for c, neibor in tqdm(ent_nei.items()):
        ...
        t += 1
        if t % 50 == 0:
            np.save('out/books_entity_embeddings_gcn.npy', all_sage_embed)
    return
...
def load_copus():
    with open('books511_text_set_BERT.pickle', 'rb') as f:
        all_triple = pickle.load(f)
    return all_triple
